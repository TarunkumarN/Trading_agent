...
const pickSymbol = row => row?.symbol || row?.stock || row?.underlying_symbol || '--';
...
function DashboardPage({ data }) {
  ...
  const latestLabel = data.is_latest_session && activeDate ? `Latest session: ${activeDate}` : 'Current session';
  return <div className="page-enter"><div className="grid-4"><Stat label="Day P&L" value={`Rs ${fmt(data.day_pnl || 0)}`} sub={latestLabel} color={(data.day_pnl || 0) >= 0 ? 'var(--green)' : 'var(--red)'} /><Stat label="Trades" value={data.total_trades || 0} sub={activeDate || 'No session'} /><Stat label="Win Rate" value={`${data.win_rate || 0}%`} /><Stat label="Portfolio" value={`Rs ${fmtINR(data.portfolio_value || 0)}`} /></div><div className="card" style={{ marginTop: 16 }}><div className="card-head"><span className="card-head-title">Intraday P&L</span></div><div style={{ height: 220 }}><ResponsiveContainer width="100%" height="100%"><LineChart data={asArray(data.pnl_curve)}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="time" /><YAxis /><Tooltip /><Line type="monotone" dataKey="pnl" stroke="var(--green)" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div></div></div>;
}
...
function ReportPage({ report }) {
  ...
  const dates = asArray(current.available_dates).length ? current.available_dates : (current.date ? [current.date] : []);
  return <div className="page-enter"><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, gap: 12, flexWrap: 'wrap' }}><div className="card-label">Daily report</div><select value={date} onChange={e => loadDate(e.target.value)}>{dates.map(d => <option key={d} value={d}>{d}</option>)}</select></div><div className="grid-4"><Stat label="Trades" value={current.total_trades || 0} sub={current.date || ''} /><Stat label="Wins / Losses" value={`${current.winning_trades || 0} / ${current.losing_trades || 0}`} /><Stat label="Day P&L" value={`Rs ${fmt(current.daily_pnl || 0)}`} color={(current.daily_pnl || 0) >= 0 ? 'var(--green)' : 'var(--red)'} /><Stat label="Portfolio" value={`Rs ${fmtINR(current.portfolio_value || 0)}`} /></div><div className="card" style={{ marginTop: 16 }}><div className="card-head"><span className="card-head-title">Performance Metrics</span></div><div className="fo-row"><span className="c-dim">Avg Profit</span><span>Rs {fmt(current.avg_profit || 0)}</span></div><div className="fo-row"><span className="c-dim">Avg Loss</span><span>Rs {fmt(current.avg_loss || 0)}</span></div><div className="fo-row"><span className="c-dim">Profit Factor</span><span>{current.profit_factor || 0}x</span></div><div className="fo-row"><span className="c-dim">Best Trade</span><span>{current.best_trade ? `${pickSymbol(current.best_trade)} (${fmt(current.best_trade.pnl || 0)})` : '--'}</span></div><div className="fo-row"><span className="c-dim">Worst Trade</span><span>{current.worst_trade ? `${pickSymbol(current.worst_trade)} (${fmt(current.worst_trade.pnl || 0)})` : '--'}</span></div></div><div className="card" style={{ marginTop: 16 }}><div className="card-head"><span className="card-head-title">Strategy Breakdown</span></div>{asArray(current.strategy_performance).length ? <div className="tbl-wrap"><table><thead><tr><th>Strategy</th><th>Trades</th><th>Win%</th><th>P&L</th></tr></thead><tbody>{current.strategy_performance.map((s, i) => <tr key={i}><td>{s.name}</td><td>{s.trades}</td><td>{s.win_rate}%</td><td style={{ color: s.pnl >= 0 ? 'var(--green)' : 'var(--red)' }}>Rs {fmt(s.pnl)}</td></tr>)}</tbody></table></div> : <div className="empty">No strategy data</div>}</div></div>;
}

function PositionsPage({ positions }) {
  const pos = asArray(positions?.positions);
  const exposure = pos.reduce((sum, p) => sum + (p.entry_price || p.entry || 0) * (p.quantity || p.qty || 0), 0);
  return <div className="page-enter"><div className="grid-3"><Stat label="Open" value={pos.length} /><Stat label="Unreal P&L" value={`Rs ${fmt(pos.reduce((s, p) => s + (p.unrealised_pnl || 0), 0))}`} /><Stat label="Exposure" value={`Rs ${fmtINR(exposure)}`} /></div><div className="card" style={{ marginTop: 16 }}>{pos.length ? <div className="tbl-wrap"><table><thead><tr><th>Symbol</th><th>Strategy</th><th>Side</th><th>Qty</th><th>Entry</th><th>Current</th><th>SL</th><th>Target</th><th>P&L</th></tr></thead><tbody>{pos.map(p => <tr key={pickSymbol(p)}><td>{pickSymbol(p)}</td><td>{p.strategy || '--'}</td><td>{p.action}</td><td>{p.quantity || p.qty}</td><td>{Number(p.entry_price || p.entry || 0).toFixed(2)}</td><td>{Number(p.current_price || 0).toFixed(2)}</td><td>{Number(p.stop_loss || p.sl || 0).toFixed(2)}</td><td>{Number(p.target || 0).toFixed(2)}</td><td style={{ color: (p.unrealised_pnl || 0) >= 0 ? 'var(--green)' : 'var(--red)' }}>Rs {fmt(p.unrealised_pnl || 0)}</td></tr>)}</tbody></table></div> : <div className="empty">No positions</div>}</div></div>;
}
...
function TradesPage({ tradesData }) {
  ...
  let trades = asArray(tradesData.trades);
...
function ChartPage() {
  ...
  const groups = [
    ['Equities', asArray(universe?.equities)],
    ['F&O', asArray(universe?.options).length ? universe.options : asArray(universe?.fno)],
    ['Commodities', asArray(universe?.commodities)],
  ];
  ...
  return <div className="page-enter"><div className="card" style={{ marginBottom: 16 }}><div className="card-head"><span className="card-head-title">Live Chart</span><span className="tag tag-active">{chartData?.instrument_type || '--'}</span></div><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}>{groups.map(([label, rows]) => <select key={label} value={rows.some(row => row.symbol === symbol) ? symbol : ''} onChange={e => e.target.value && setSymbol(e.target.value)}><option value="">{label}</option>{rows.map(row => <option key={row.symbol} value={row.symbol}>{row.symbol}</option>)}</select>)}</div><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}><select value={underlying} onChange={e => setUnderlying(e.target.value)}>{asArray(universe?.fno).map(row => <option key={row.symbol} value={row.symbol}>{row.symbol}</option>)}</select><select value={optionSide} onChange={e => setOptionSide(e.target.value)}><option value="CALL">CALL</option><option value="PUT">PUT</option></select><button className="fo-btn" onClick={() => fetch(`${API}/api/chart/data?symbol=${encodeURIComponent(symbol)}`).then(r => r.json()).then(setChartData).catch(() => {})}>Refresh</button></div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}><span className="tag tag-ok">Close</span><span className="tag tag-active">EMA20</span><span className="tag tag-warn">EMA50</span><span className="tag tag-paper">VWAP</span><span className="tag tag-info">{commodityBadge}</span></div><div style={{ width: '100%', height: 420 }}>{chartRows.length ? <ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartRows}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="time" hide /><YAxis domain={['auto', 'auto']} /><Tooltip /><Line type="monotone" dataKey="close" stroke="#22c55e" dot={false} strokeWidth={2} name="Close" /><Line type="monotone" dataKey="ema20" stroke="#06b6d4" dot={false} strokeWidth={2} name="EMA20" /><Line type="monotone" dataKey="ema50" stroke="#eab308" dot={false} strokeWidth={2} name="EMA50" /><Line type="monotone" dataKey="vwap" stroke="#a855f7" dot={false} strokeWidth={2} name="VWAP" /></ComposedChart></ResponsiveContainer> : <div className="empty">No chart data</div>}</div></div><div className="grid-3"><Stat label="Symbol" value={chartData?.symbol || symbol} /><Stat label="Instrument" value={chartData?.instrument_type || '--'} /><Stat label="Candles" value={asArray(chartData?.candles).length || 0} /></div><div className="card" style={{ marginTop: 16, height: 220 }}>{chartRows.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={chartRows}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="time" hide /><YAxis /><Tooltip /><Bar dataKey="volume" fill="#334155" name="Volume" /></BarChart></ResponsiveContainer> : <div className="empty">No volume data</div>}</div></div>;
}
