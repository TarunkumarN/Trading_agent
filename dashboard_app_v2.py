...
.nav-item{display:flex;align-items:center;gap:9px;padding:8px 16px;cursor:pointer;color:var(--muted2);font-size:.74rem;border-left:2px solid transparent;transition:all .15s;}
...
.metric{font-family:'Barlow',sans-serif;font-size:1.75rem;font-weight:800;line-height:1;letter-spacing:-.5px;}
...
.card-head-title{font-size:.74rem;font-weight:600;}
...
td{padding:7px 10px;border-bottom:1px solid rgba(22,32,48,.5);}
...
.tab{padding:6px 16px;font-size:.7rem;font-weight:600;cursor:pointer;color:var(--muted2);background:transparent;border:none;letter-spacing:.3px;}
...
<div id="app" style="display:none;">
  ...
  <div class="main">
    <div class="topbar">
      ...
      <div class="tb-right">
        <span style="color:var(--muted2);">NSE <span class="blink" id="mktDot" style="color:var(--green);">●</span></span>
    ...
    <div class="content">
      ...
      <div class="page active" id="page-dashboard">
        <div class="grid-4">
          <div class="card card-accent ca-green fadein">
            <div class="card-title">Day P&L</div>
            <div class="metric" id="d-pnl" style="color:var(--green)">₹+0.00</div>
      ...
      <div class="page" id="page-trades">
        ...
        <div style="display:flex;gap:10px;margin-bottom:10px;align-items:center;">
          <select id="tradeFilter" onchange="renderTrades()" style="background:var(--bg3);border:1px solid var(--border2);color:var(--text);padding:6px 10px;border-radius:5px;font-family:inherit;font-size:.7rem;">
      ...
      <div class="page" id="page-risk">
        ...
        <div class="card fadein" style="animation-delay:.3s">
          ...
          <div id="r-status" style="padding:14px;text-align:center;">
            ...
            <div style="font-size:.7rem;color:var(--muted);margin-top:5px;" id="r-status-sub">All systems normal</div>
          </div>
        </div>
      </div>
...
function showApp(){
  ...
  initCharts(); refresh(); loadHealth(); loadRisk(); loadStrategies();
...
function updateDashboard(){
  ...
  document.getElementById('d-watchlist').innerHTML=wl.length?wl.map(s=>'<span class="chip">'+s+'</span>').join(''):'<span class="c-muted" style="font-size:.7rem">No watchlist loaded</span>';
  // P&L chart
  ...
  document.getElementById('d-openpos').innerHTML=op.length
    ?'<table><thead><tr><th>Stock</th><th>Side</th><th>Qty</th><th>Entry</th><th>Unrealised</th></tr></thead><tbody>'+
      op.map(pos=>'<tr><td style="font-weight:700;color:var(--amber);">'+pos.stock+'</td><td><span class="tag tag-'+(pos.action.toLowerCase())+'">'+pos.action+'</span></td><td class="c-muted">'+pos.qty+'</td><td>₹'+pos.entry.toFixed(2)+'</td><td style="color:'+(pos.unrealised_pnl>=0?'var(--green)':'var(--red)')+';">'+fmt(pos.unrealised_pnl)+'</td></tr>').join('')+'</tbody></table>'
  ...
  document.getElementById('d-recent').innerHTML=rec.length
    ?'<table><thead><tr><th>Stock</th><th>P&L</th><th>Reason</th><th>Time</th></tr></thead><tbody>'+
      rec.map(t=>{
        ...
        return'<tr><td style="font-weight:700;color:var(--amber);">'+t.stock+'</td><td style="color:'+(t.pnl>=0?'var(--green)':'var(--red)')+';">'+fmt(t.pnl)+'</td><td><span class="tag '+rc+'">'+rl+'</span></td><td class="c-muted">'+t.exit_time+'</td></tr>';
      }).join('')+'</tbody></table>'
...
function renderTrades(){
  ...
  document.getElementById('t-table').innerHTML='<table><thead><tr><th>Stock</th><th>Action</th><th>Qty</th><th>Entry</th><th>Exit</th><th>P&L</th><th>Score</th><th>Reason</th><th>In</th><th>Out</th></tr></thead><tbody>'+
    [...filtered].reverse().map(t=>{
      ...
      return'<tr><td style="font-weight:700;color:var(--amber);">'+t.stock+'</td><td><span class="tag tag-'+(t.action.toLowerCase())+'">'+t.action+'</span></td><td class="c-muted">'+t.qty+'</td><td>₹'+t.entry.toFixed(2)+'</td><td>₹'+t.exit.toFixed(2)+'</td><td style="color:'+(t.pnl>=0?'var(--green)':'var(--red)')+';">'+fmt(t.pnl)+'</td><td><span class="tag '+(t.score>=8?'tag-ok':'tag-active')+'">'+(t.score||'--')+'/10</span></td><td><span class="tag '+rc+'">'+rl+'</span></td><td class="c-muted">'+t.entry_time+'</td><td class="c-muted">'+t.exit_time+'</td></tr>';
    }).join('')+'</tbody></table>';
}
...
function updatePositions(){
  ...
  document.getElementById('pos-table').innerHTML='<table><thead><tr><th>Stock</th><th>Side</th><th>Qty</th><th>Entry</th><th>Current</th><th>SL</th><th>Target</th><th>Phase</th><th>Unrealised</th><th>Score</th><th>Time</th></tr></thead><tbody>'+
    op.map(pos=>'<tr><td style="font-weight:700;color:var(--amber);">'+pos.stock+'</td><td><span class="tag tag-'+(pos.action.toLowerCase())+'">'+pos.action+'</span></td><td class="c-muted">'+pos.qty+'</td><td>₹'+pos.entry.toFixed(2)+'</td><td style="font-weight:700;">₹'+(pos.current||pos.entry).toFixed(2)+'</td><td class="c-red">₹'+pos.sl.toFixed(2)+'</td><td class="c-green">₹'+pos.target.toFixed(2)+'</td><td class="c-muted" style="font-size:.62rem;">'+(pos.sl_phase||'INITIAL')+'</td><td style="color:'+(pos.unrealised_pnl>=0?'var(--green)':'var(--red)')+';">'+fmt(pos.unrealised_pnl)+'</td><td><span class="tag tag-active">'+(pos.score||'--')+'/10</span></td><td class="c-muted">'+pos.time+'</td></tr>').join('')+
  '</tbody></table>';
}
  ...
  fetch('/api/premarket').then(r=>r.json()).then(d=>{
    ...
    function moverTable(arr){
      ...
      return'<table><thead><tr><th>Symbol</th><th>Price</th><th>Gap%</th><th>Volume</th><th>Momentum</th></tr></thead><tbody>'+
        arr.map(m=>'<tr><td style="font-weight:700;color:var(--amber);">'+m.symbol+'</td><td>₹'+m.price+'</td><td style="color:'+(m.gap_pct>=0?'var(--green)':'var(--red)')+';">'+(m.gap_pct>=0?'+':'')+m.gap_pct+'%</td><td class="c-muted">'+m.vol_score+'</td><td><span class="tag '+(m.momentum.includes('Bullish')?'tag-bullish':'m.momentum.includes("Bearish")?'tag-bearish':'tag-neutral')+'">'+m.momentum+'</span></td></tr>').join('')+'</tbody></table>';
    }
